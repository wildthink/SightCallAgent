...
public protocol SightCallProvider {
    var sightCallSDK: SightCallSDK! { get }
}
...
extension SightCallSDK {
    ...
    func open(url: String, withRegistrationReference ref: String? = nil)
    {
        if (ls_sdk.status != .idle &&
                ls_sdk.status != .agentConnected)
        {
            print ("Already busy")
//            [self logFromMethod:[NSString stringWithFormat:@"[%@ %@]", self.class, NSStringFromSelector(_cmd)] message:@"Already busy %d", (int)self.universal.status, nil];
        } else if let ref = ref ?? notificationToken {
            ls_sdk.agentHandler?.notificationReference = ref
            ls_sdk.start(with: url)
        } else {
            print ("WARNING", "Missing Notification Reference", #function)
        }
    }
